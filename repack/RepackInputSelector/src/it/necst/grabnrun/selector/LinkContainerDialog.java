...
public class LinkContainerDialog extends Dialog {
	...
	private TextField oneCertPathField;
	...
	public LinkContainerDialog(	Frame owner, 
        ...
        JLabel explValManyCertLabel = new JLabel("<html>Here each container must be linked with a not empty URL pointing to a remote certificate.<br>HTTPS protocol is required for all the provided entries.</html>", JLabel.CENTER);
        ...
        contAndCertPan.add(new JLabel("<html>Container URL</html>", JLabel.CENTER));
		...
		for (int i=0; i < MAX_NUMBER_OF_MAP_ENTRIES - validURLContSet.size(); i ++) {
	...
	private void validateInput() {
		...
		case VAL_MANY_CERTS:
			...
			if (isUserInputValid) {
				...
				JOptionPane.showMessageDialog(this, "At least one of the entries was empty or did not match a remote URL using HTTPS!", "Invalid certificate URL", JOptionPane.ERROR_MESSAGE);
			}
